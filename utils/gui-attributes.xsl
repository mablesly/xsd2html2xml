...
<xsl:stylesheet
	...
	<xsl:template name="set-type-defaults">
		...
		<xsl:choose>
			...
			<xsl:when test="$type = 'hexbinary'">
			    <xsl:call-template name="set-pattern">
					<xsl:with-param name="prefix">[0-9a-fA-F]</xsl:with-param>
					<xsl:with-param name="root-document" select="$root-document" />
					<xsl:with-param name="root-path" select="$root-path" />
					<xsl:with-param name="root-namespaces" select="$root-namespaces" />
					<xsl:with-param name="namespace-documents" select="$namespace-documents" />
					<xsl:with-param name="length-factor">2</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="set-pattern">
	...
	<xsl:template name="set-pattern">
        ...
        <xsl:param name="length-factor">1</xsl:param>
			...
			<xsl:attribute name="pattern">
				<xsl:choose>
					...
					<xsl:when test="not($length='')">
						<xsl:value-of select="concat($prefix,'{',$length * $length-factor,'}')" />
					</xsl:when>
	...
	<xsl:template match="xs:length|xs:maxLength">
        <xsl:variable name="length-factor">
            <xsl:choose>
		...
		<xsl:attribute name="maxlength">
			<xsl:value-of select="@value * $length-factor"/>
		</xsl:attribute>
	</xsl:template>
